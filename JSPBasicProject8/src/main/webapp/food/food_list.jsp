...
<%
	...
	/*
		1. 생성 부분
			...
			setPath() : 쿠키 저장 경로 지정
			setMaxAge() : 쿠키 저장 기간
			==> client로 쿠키 전송 response.addCookie()
		2. 쿠키 읽기
			Cookie[] cookies=request.getCookies();
			getName() => key를 읽어온다
			getValue() => 값을 읽어온다
  ...
  <div class="container">
    ...
    <div style="clear: both;"></div>
      ...
      <%--
      		...
      		1. 쿠키 사용법 ==> 쿠키의 단점 : 저장되는 데이터가 문자열만, 세션=>object 가능
      			- 쿠키 생성
      ...
      <div>
        <%
        	...
        	for (FoodVO vo:cList) {
        		if(k>9) break;
	    %>
	    		<a href="food_detail.jsp?no=<%=vo.getFno() %>"><img src="<%=vo.getPoster().substring(0, vo.getPoster().indexOf("^")) %>" width=100 height="100"></a>
	    <%        		
	    		k++;
        	}
        %>
