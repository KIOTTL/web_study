...
public class ReserveModel {
	...
	public String reserve_date(HttpServletRequest request, HttpServletResponse response) {
		...
		int[] days=new int[32];	// 자른 개수
		
		int i=0;
		while(st1.hasMoreTokens()) {	// 자른 개수만큼
			int d=Integer.parseInt(st1.nextToken());
			if (d>=day) {	// 오늘 이후
				days[d]=1;
			}
		}
		// 0이면 예약 없는 날, 1이면 예약 있는 날
    ...
    public String rserve_cancel(HttpServletRequest request,HttpServletResponse response) {
    	String no=request.getParameter("no");
