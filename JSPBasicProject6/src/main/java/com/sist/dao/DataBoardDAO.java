...
public class DataBoardDAO {
	...
	public void databoardInsert(DataBoardVO vo) {
		try {
			conn=dbconn.getConnection();
			String sql = "INSERT INTO databoard(no, name, subject, content, pwd, filename, filesaize) "
	...
	public DataBoardVO databoardDetail(int no) {
		...
		try {
			...
			sql = "SELECT no, name, subject, content, regdate, hit, filename, filesaize "
					+"FROM databoard "
					+"WHERE no=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, no);
